    ...
    public class Parametros
    {
        ...
        private void SalvaValorPropriedade(PropertyInfo pInfo, string valorPropriedade)
        {
            ...
            else if (propertyType == typeof(string))
            {
                ...
                valor = long.Parse(valorPropriedade.ToString());
        ...
        public void Load(string[] args)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                {
                    if (++i >= args.Length)
                        GeraErro("O número de parametros é inválido.");
...
USO:
    SvnError.exe [-Modificador] [Valor Parametro]
...
onde os modificadores e valores válidos são:", mensagemErro);
        ...
        [CommandParameter("-path", "."), Description("Caminho do projeto")]
        public string Pasta { get; set; }

        [CommandParameter("-minrev", "-1"), Description("Numero da versao minima de busca")]
        public long RevisaoMinima { get; set; }

        [CommandParameter("-maxrev", "-1"), Description("Numero da versao maxima de busca")]
        public long RevisaoMaxima { get; set; }

        [CommandParameter("-build", "builda.bat"), Description("Nome do comando de build (e.g. build.bat)")]
        public string BuildCommand { get; set; }

        [CommandParameter("-buildargs", ""), Description("Argumentos do comando de build (e.g. -t)")]
        public string BuildArguments { get; set; }


        [CommandParameter("-test", "test.bat"), Description("Nome do comando de teste (e.g. teste.bat)")]
        public string TestCommand { get; set; }

        [CommandParameter("-testargs", ""), Description("Argumentos do comando de teste (e.g. -t)")]
